  ...
  `;
      ...
      $(".click--modal-hp").off("click").on("click", function () {
        ...
        $(".showing_modal_pop_up .close").off("click").on("click", function () {
          ...
          $("#forgot-password-block input[name=\"email\"]").val("");
          $("#forgot-password-block input[name=\"known_member\"]").val("");
        });
...
modal_1.click(function (e) {
  ...
  // "Forgot password?" (member modal). Checks the typed email, without ever
  // sending it anywhere, against a list of SHA-256 hashes of emails that
  // already went through the members form (Company field) successfully.
  // A match sets the hidden "known_member" field so the "forgot access code"
  // webform's email handler sends the code; the confirmation message shown to
  // the visitor is the same either way, so the form never reveals whether a
  // given email belongs to a member.
  (function () {
    // Public-domain SHA-256 (ASCII/byte-string input).
    ...
    function hashEmail(email) {
      var normalized = $.trim(email).toLowerCase();
      return sha256(unescape(encodeURIComponent(normalized)));
    }

    var verifiedHashes = [];
    $.getJSON('/themes/custom/cfc/CFC/assets/js/cfc_verified_emails.json', function (data) {
      verifiedHashes = data || [];
    });

    $(document).on('click', '#forgot-password-btn', function () {
      $('#forgot-password-block').slideToggle(150);
    });

    $(document).on('input change', '#forgot-password-block input[name="email"]', function () {
      var typed = $.trim($(this).val());
      var known = typed !== '' && verifiedHashes.indexOf(hashEmail(typed)) !== -1;
      $('#forgot-password-block input[name="known_member"]').val(known ? '1' : '0');
    });
  })();
